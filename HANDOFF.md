...
- **Open / next:** the shipping demo still runs Tier-1 (`tier2` defaults off) — a frontend toggle to *show* the GR/Yarkovsky shift live is the natural follow-up but not built. Remaining Tier-2 force work: SRP, J2 (negligible for the heliocentric Sun — low priority), the 16 `sb441` bodies promoted from scenery to force perturbers, and Pluto-in-shipping (needs a DE441-consistent Pluto GM — `pck11.pca` carries none). With GR **and** Yarkovsky now both available on the real field, the capstone is finally reachable: validate a real NEO's *own* integration against its Horizons trajectory (the sampled `.neo` tables are JPL's truth to compare against).
...
- **Open / next:** a frontend toggle to *show* the belt (and GR/Yarkovsky) shift live is still the natural follow-up. Remaining Tier-2 force menu: SRP, J2 (low priority), Pluto-in-shipping (needs a DE441-consistent Pluto GM). The residual floor is now GR-of-the-planets + JPL's radial A1, which we do not model.

...
### Tier 2 begun — 2026-07-20 session (1PN relativity term)
...
- **Open / next:** the term is validated in isolation but **not yet wired into the shipping scenario** — that is the deliberate next commit (§206: validate per term first). Wiring = add the 1PN term to the real-field force model behind a toggle, then re-check the threat b-plane is *unchanged* with GR off and *shifted* with GR on. Then Yarkovsky (the decade-scale along-track dominator), SRP, J2, the 16 asteroid perturbers as force terms, and Pluto-in-shipping — the rest of the Tier-2 body of work — before a real NEO's *own* integration can be validated against Horizons.

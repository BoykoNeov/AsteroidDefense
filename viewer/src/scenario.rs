...
//!
//! This module turns [`asteroid_core`]'s kernel-free deflection machinery into a
...
//! how early it is applied. It is **egui-free** on purpose so Commit B1 (the
//! `curve` binary) can prove the physics before any renderer links.
//!
...
use asteroid_core::forces::point_mass::PointMassGravity;
use asteroid_core::geometry::BPlaneEncounter;
use asteroid_core::perturber_field::{tier1_perturber_field, EphemerisPerturber};
use asteroid_core::{
...
/// deflected closest approach. Shared by the viewer and its test so the test
...
/// the binary and the future egui app surface one error type.
#[derive(Debug)]
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ScenarioError::MissingKernelEnv(v) => {
                write!(f, "environment variable {v} is not set (kernel path required)")
            }
            ...
            ScenarioError::NominalNotAHit(m) => write!(f, "designed impact did not round-trip: {m}"),
...
impl RealFieldScenario {
    ...
    /// bracket+bisect solve, so this is the expensive call — the whole reason
    /// Commit B1 times it before the egui app decides on-thread vs. background.
    pub fn sweep(
        &self,
        leads_periods: &[f64],
        target_perigee_m: f64,
    ) -> Result<Vec<CurvePoint>, ScenarioError> {
    ...
    /// costs seconds. The egui app must not pay that per nudge: it builds one
    ...
    /// the sub-second per-nudge cost the `probe_prop` binary timed at ~1–2 orbits
    /// of lead. `ds` must be one this scenario produced (via
...
mod tests {
    ...
    /// pinned in `asteroid_core`'s own tests.
    #[test]
    fn encounter_frame_track_agrees_with_reported_perigee() {
        ...
        let deflection_epoch = sc
            .impact_epoch()
            .shifted_by_seconds(-sc.period_seconds);
        let ds = sc.deflection().expect("deflection scenario");
        let seed = ds.nominal().state_at(deflection_epoch).expect("nominal state");

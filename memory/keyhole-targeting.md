...
**2026-09-07 - the frontend's keyhole alert rule changed.**
`godot/scripts/sim.gd` no longer has `KEYHOLE_ALERT_WIDTHS := 4.0`; five flown
keyholes rejected it (it missed three of them). The rule is now additive,
`|distance_km| <= KEYHOLE_PLACEMENT_KM + width_km/2` with
`KEYHOLE_PLACEMENT_KM := 100.0`, because a door has a real width *and* a
displaced centre and the two errors add. **The 100 km is chosen, not measured** -
every aim was at one point per circle (xi = 6690 km). It is bracketed from
above by circle crowding: at `KEYHOLE_MAX_YEARS` = 7 the closest two drawn
circles are 402 km apart, but **at 20 years the tightest pair is 3.0 km**, so
raising the horizon forces this constant down. The panel now names the circle
nearest in **kilometres** (not in half-widths), and it no longer claims "RETURN
SET UP" for a plan inside a drawn door - four of five flown doors did not
contain their circle, so that was never sufficient. Full measurement in
[[keyhole-placement]].

...
- **Open / next:** *(all resolved — the shipping demo still defaults `tier2` off, but the live frontend toggle, SRP, the 16 `sb441` perturbers, and the Horizons capstone all landed by 2026-07-21; see **Tier 2 complete** below.)* Remaining: J2 (low priority) and Pluto-in-shipping (needs a DE441-consistent Pluto GM — `pck11.pca` carries none).
...
- **Open / next:** ~~a frontend toggle to show the belt shift live~~ and ~~SRP~~ both landed the following session — see *Tier 2 complete* below. Remaining: J2 (low priority — negligible heliocentrically) and Pluto-in-shipping (needs a DE441-consistent Pluto GM). The residual floor is now GR-of-the-planets + JPL's radial A1, which we do not model.
...
- **Where the force model stands.** The Tier-2 term list (§166) is complete but for **J2** (deliberately deferred — negligible for the heliocentric Sun, only matters for very close Earth flybys / keyhole geometry, which is Tier-3's regime) and **Pluto-in-shipping** (still needs a DE441-consistent Pluto GM — `pck11.pca` carries none). Everything else ASSIST carries, we carry, validated per-term against the closed form or Horizons. **The next spec beats are the Phase-2 deflection-method spectrum (§5: nuclear standoff + gravity tractor, beyond the MVP's kinetic-only), Lambert/porkchop mission design (§8), and Tier 3 uncertainty (§175 — covariance → b-plane → impact probability, keyholes, and where the deferred b-vector sign/ξζ convention at the open-questions list finally gets settled).**
